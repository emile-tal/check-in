...
interface Props {
    ...
    updateTurnsLeft: () => void
}

export function Game({ updateTotalPoints, updateTurnsLeft }: Props) {
    ...
    return (
        <div className='game'>
            <div className='game__draw-container'>
                <div className='game__deck-container'>
                    <img src={deck} className='game__draw-pile' onClick={drawFromDeck} />
                    {(takeFromDeck || newOpenCard) && <img src={deck} className='game__draw-card' ref={drawCardTarget} />}
                    {takeFromDeck && <img src={selectedDrawTile[0]} className='game__draw-pile-card' ref={drawnCardTarget} />}
                </div>
                <div className='game__draw'>
                    <motion.img
                        src={lobby}
                        className={`game__lobby-draw ${selectedDrawTile[0] === lobby ? 'game__lobby-draw--selected' : ''}`}
                        onClick={() => selectDrawTile(lobby, -1)} animate={{ scale: selectedDrawTile[0] === lobby ? (isMobile ? 1.1 : 1.2) : 1 }}
                        whileTap={{ scale: 0.95 }}
                    />
                    {drawTiles.map((drawTile, index) => {
                        ...
                        return (
                            < motion.img
                                ...
                                ref={openTileTarget}
                                src={drawTile}
                                className={`game__draw-tile ${selectedDrawTile[1] === index ? 'game__draw-tile--selected' : ''}`}
